...
func (oq *OrganizationQuery) QueryResources() *ResourceQuery {
	query := &ResourceQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) WithResources(opts ...func(*ResourceQuery)) *OrganizationQuery {
	query := &ResourceQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context) ([]*Organization, error) {
	...
	if query := oq.withResources; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Organization)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Resource(func(s *sql.Selector) {
			s.Where(sql.InValues(organization.ResourcesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
