...
func (rq *ResourceQuery) QuerySlots() *SlotQuery {
	query := &SlotQuery{config: rq.config}
...
func (rq *ResourceQuery) QueryBookings() *BookingQuery {
	query := &BookingQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ResourceQuery) QueryUnavailabilities() *UnavailabilityQuery {
	query := &UnavailabilityQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ResourceQuery) QueryOrganization() *OrganizationQuery {
...
func (rq *ResourceQuery) Clone() *ResourceQuery {
	...
	return &ResourceQuery{
		...
		withSlots:            rq.withSlots.Clone(),
		withBookings:         rq.withBookings.Clone(),
		withUnavailabilities: rq.withUnavailabilities.Clone(),
...
func (rq *ResourceQuery) WithSlots(opts ...func(*SlotQuery)) *ResourceQuery {
	query := &SlotQuery{config: rq.config}
...
func (rq *ResourceQuery) WithBookings(opts ...func(*BookingQuery)) *ResourceQuery {
	query := &BookingQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *ResourceQuery) WithUnavailabilities(opts ...func(*UnavailabilityQuery)) *ResourceQuery {
	query := &UnavailabilityQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withUnavailabilities = query
	return rq
}

// WithOrganization tells the query-builder to eager-load the nodes that are connected to
// the "organization" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *ResourceQuery) WithOrganization(opts ...func(*OrganizationQuery)) *ResourceQuery {
	query := &OrganizationQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *ResourceQuery) sqlAll(ctx context.Context) ([]*Resource, error) {
	...
	if query := rq.withSlots; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Resource)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Slots = []*Slot{}
		}
		query.Where(predicate.Slot(func(s *sql.Selector) {
			s.Where(sql.InValues(resource.SlotsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "resourceId" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.Slots = append(node.Edges.Slots, n)
		}
	}
	...
	if query := rq.withBookings; query != nil {
		...
		query.Where(predicate.Booking(func(s *sql.Selector) {
			s.Where(sql.InValues(resource.BookingsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := rq.withUnavailabilities; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Resource)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Unavailability(func(s *sql.Selector) {
			s.Where(sql.InValues(resource.UnavailabilitiesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
