...
func (sq *SlotQuery) QueryResource() *ResourceQuery {
	query := &ResourceQuery{config: sq.config}
...
// Returns a *NotSingularError when exactly one Slot entity is not found.
...
// Returns a *NotSingularError when exactly one Slot ID is not found.
...
func (sq *SlotQuery) Clone() *SlotQuery {
	...
	return &SlotQuery{
		...
		withResource: sq.withResource.Clone(),
...
func (sq *SlotQuery) WithResource(opts ...func(*ResourceQuery)) *SlotQuery {
	query := &ResourceQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withResource = query
	return sq
}
...
func (sq *SlotQuery) sqlAll(ctx context.Context) ([]*Slot, error) {
	...
	if query := sq.withResource; query != nil {
